...
def evaluate(model, dataset,
    ...
    with tf.Graph().as_default() as g:
        ...
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=yt_one, logits=y))
        accuracy=tf.reduce_mean(tf.cast(tf.equal(yt, tf.cast(tf.argmax(y, axis=1),dtype=tf.int32)),dtype=tf.float32))
